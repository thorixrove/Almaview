...
export const runtime = "nodejs"; // pastikan pakai Node runtime, bukan Edge, karena butuh 'zlib'

async function readBody(request) {
    const contentEncoding = request.headers.get("content-encoding") ?? "";
    const buffer = Buffer.from(await request.arrayBuffer());

    console.log(`[stream-webhook] Content-Encoding: "${contentEncoding}" | bytes: ${buffer.length}`);
    ...
    try {
        ...
        console.log(`[stream-webhook] Decompression failed, trying raw utf-8`);
        return buffer.toString("utf-8");
    }
}
...
export async function POST(request) {
    ...
    try {
        ...
        console.log(
            `[stream-webhook] Ignoring non-JSON request (bukan dari Stream). Preview: ${rawText.slice(0, 80)}`
        );
    ...
    console.log(`[stream-webhook] Full payload keys: ${Object.keys(body).join(", ")}`);
    ...
    try {
        ...
        if (eventType === "call.transcription_ready") {
            ...
            // 2. Parse JSONL into readable converstion
            ...
            // 3. Generate feedback via Groq
            console.log(
                `[stream-webhook] Sending transcript to Groq (openai/gpt-oss-20b)...`
            );
...
Analyze the candidate's performance. Respond ONLY with a valid JSON object, no markdown, no backticks, no explanation:
{
  ...
  "overallRating": "POOR or AVERAGE or GOOD or EXCELLENT"
            ...
            console.log(
                `[stream-webhook] Groq raw response:\n${raw.slice(0, 500)}${raw.length > 500 ? "..." : ""
                }`
            );
            ...
            // 4. Write to DB - upsert handles concurrent webhook retries cleanly
            ...
            // Credit trnsaction is outside the main transaction so we cam check first
